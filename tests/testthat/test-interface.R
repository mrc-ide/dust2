...
test_that("Can unpack history", {
  sys <- dust_system_create(sir(), list(), n_particles = 10)
  dust_system_set_state_initial(sys)
  s <- dust_system_simulate(sys, 0:20)
  ## I think that this is a bug in monty; we've preserved a state
  ## dimension here that we should know that we should be dropping,
  ## because we have things down as integer(0)
  s2 <- dust_unpack_state(sys, s)
})

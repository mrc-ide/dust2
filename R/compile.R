...
dust_generate_cpp <- function(system, config, data) {
  includes <- c("cpp11.hpp", "dust2/r/discrete/system.hpp")
  code <- substitute_dust_template(data, "discrete/system.cpp")
  ...
  if (config$has_compare) {
    includes <- c(includes,
                  "dust2/r/discrete/filter.hpp",
                  "dust2/r/discrete/unfilter.hpp")
    code <- c(code,
              "",
              substitute_dust_template(data, "discrete/compare.cpp"))
  }

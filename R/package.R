...
package_validate_namespace <- function(path, name, call) {
  ...
  package_validate_namespace_usedynlib(exprs, name, call)
}


package_validate_namespace_usedynlib <- function(exprs, name, call) {
  for (e in exprs) {
    if (rlang::is_call(e, "useDynLib")) {
      ...
      cli::cli_abort(
        "Found a 'useDynLib()' call in NAMESPACE, but not for '{name}'",
        call = call)
    }
  }
  cli::cli_abort("Did not find a 'useDynLib()' call in NAMESPACE", call = call)
}
...
package_generate <- function(filename, call) {
  ...
  data <- dust_template_data(config$name, config$class)
